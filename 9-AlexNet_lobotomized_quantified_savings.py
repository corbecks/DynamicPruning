...
def filters_to_zero(activation_means, activation_deviations, cluster, activation_cutoff=0.1, std_cuttoff=0.25, default_pruning = False, combined_pruning = False):
    filter_index_to_zero = []
    activations = []
    deviations = []

    if(combined_pruning):
        activations_all = means_all
        deviations_all = stds_all
        activations_cluster = activation_means[cluster]
        deviations_cluster = activation_deviations[cluster]

        for i, activation in enumerate(activations_all):
            if ((activations_all[i] < activation_cutoff) and (deviations_all[i] < std_cuttoff)
                    or (activations_cluster[i] < activation_cutoff) and (deviations_cluster[i] < std_cuttoff)):
                filter_index_to_zero.append(i)
    else:
        if (default_pruning):
            activations = means_all
            deviations = stds_all
        else:
...
def count_zero_mults(module, input, output):
    """
    A forward-hook that, every time module(input) is called,
    counts how many of the underlying multiplications
    were zero * something.
    """
    global zero_mults
    x = input[0]
    ...
    if isinstance(module, nn.Conv2d):
        # 1) unfold the input into patches of shape
        #    (batch, in_ch * kH * kW, L), where L = # of output pixels
        patches = F.unfold(x,
                           kernel_size=module.kernel_size,
                           dilation=module.dilation,
                           padding=module.padding,
                           stride=module.stride)
        # 2) flatten the weight into (out_ch, in_ch * kH * kW)
        W_flat = module.weight.view(module.out_channels, -1)
        # 3) broadcast multiply:
        #    W_flat shape → (1, out_ch,       patch_size, 1)
        #    patches    →   (batch, 1,        patch_size, L)
        #    result     →   (batch, out_ch,   patch_size, L)
        prod = W_flat.unsqueeze(0).unsqueeze(3) * patches.unsqueeze(1)
        # 4) count zeros
        zero_mults += int((prod == 0).sum())

    elif isinstance(module, nn.Linear):
        # flatten inputs to (batch, in_features)
        x_flat = x.view(x.size(0), -1)
        # weight already (out_features, in_features)
        W_flat = module.weight
        # broadcast to (batch, out_features, in_features)
        prod = x_flat.unsqueeze(1) * W_flat.unsqueeze(0)
        zero_mults += int((prod == 0).sum())
...
cluster_classifier_model = FemtoNet()

cluster_classifier_state_dict = torch.load(f"cluster_classifiers/femtonet/femtonet_final_weights.pt", weights_only=False)
...
# 1) register the hook on every conv / linear
for m in model.modules():
    if isinstance(m, (nn.Conv2d, nn.Linear)):
        m.register_forward_hook(count_zero_mults)
...
grid_zero_products = np.empty((5, 5))
grid_ave_zero_products = np.empty((5, 5))
    ...
    for inputs, labels, clusters in data_loader:
        ...
        if(cuttoff_increment==24):
            ...
            if(images_seen_increment%100 == 0):
                torch.save(grid_accuracy, f"run2_combined_femto_conventional_accuracy_vs_cuttoffs_N_{images_seen_increment}.pt")
                torch.save(grid_ave_zero_products, f"run2_combined_femto_conventional_zero_products_vs_cuttoffs_N_{images_seen_increment}.pt")
                torch.save(grid_ave_filters_removed, f"run2_combined_femto_conventional_filters_removed_vs_cuttoffs_N_{images_seen_increment}.pt")
                ...
                plt.colorbar()
                plt.title(f"Accuracy vs Cuttoffs w Combined Conv. and Femtonet Cluster Pruning (N={images_seen_increment} images)\n")
                plt.show()

                # Display the data as an image with a chosen colormap
                plt.imshow(grid_ave_zero_products,
                           cmap='viridis',
                           interpolation='none',
                           extent=(standard_deviation_cuttoffs[0],
                                   standard_deviation_cuttoffs[-1],
                ...
                plt.xlabel("Mean Activation Cuttoff")
                plt.colorbar()
                plt.title(f"Multiplies by Zero w Combined Conv. and Femtonet Cluster Pruning (N={images_seen_increment} images)\n")
                plt.show()
                ...
                plt.xlabel("Mean Activation Cuttoff")
                plt.colorbar()
                plt.title(f"Filters Removed w Combined Conv. and Femtonet Cluster Pruning (N={images_seen_increment} images)\n")
                plt.show()
        ...
        grid_zero_products[std_dev_increment][mean_act_increment] += num_zero_products
        ...
        grid_ave_zero_products[std_dev_increment][mean_act_increment]   = grid_zero_products[std_dev_increment][mean_act_increment]/ grid_total[std_dev_increment][mean_act_increment]
